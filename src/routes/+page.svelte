...
<div class="h-full flex flex-col items-center justify-evenly">
    <div class="relative">  
        <div class="animate-skew absolute w-full h-full border-2 border-current top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] skew-x-[-5deg] skew-y-[-5deg] scale-[135%]" in:fade={{delay: 250, duration: 1000, easing: cubicOut}}></div>
    ...
    <p class="px-0 flex" in:fly={{delay: 750, duration: 1000, easing: cubicOut}}>
        ...
        {#key sentenceIndex}
            <strong
                in:typewriter={{delay: 825, speed: 0.75}}
    ...
    <div class="w-min flex gap-[7.5vw]" in:fly={{delay: 300, duration: 1000, y: 100, easing: cubicOut}}>
        <button class="btn lg:btn-lg variant-filled-primary" on:click={() => openContactModal()}>Lets Talk!</button>
        <a class="btn lg:btn-lg variant-filled-secondary" href="Hugo_Korte_Resume_2023-25-1.pdf" download="Hugo_Korte_Resume_2023-25-1">Resume</a>
    </div>
</div>
