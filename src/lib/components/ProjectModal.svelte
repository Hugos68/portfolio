    ...
    <div class="card overflow-hidden">
        <header class="relative">
            <Carousel let:showPrevPage let:showNextPage>
                <button slot="prev" on:click={showPrevPage} class="z-[1] absolute btn md:btn-sm h-full left-0 top-[50%] translate-y-[-50%]">
                    <svg class="w-10 md:w-12 p-2 pl-1 variant-ghost rounded-full aspect-square text-dark-token" viewBox="0 0 24 24" fill="currentColor">
                ...
                {#if $modalStore[0]}
                    ...
                    {/each}
                {/if}
                <button slot="next" on:click={showNextPage} class="z-[1] absolute btn md:btn-sm h-full right-0 top-[50%] translate-y-[-50%] ">
